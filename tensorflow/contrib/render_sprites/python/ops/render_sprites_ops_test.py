...
"""Tests for contrib.render_sprites.python.ops.render_sprites_ops."""
...
from tensorflow.contrib import render_sprites
...
class ResamplerTest(test.TestCase):
  ...
  def _test_op_forward_pass(self, on_gpu, dtype, tol):
    ...
    with self.test_session(use_gpu=on_gpu, force_gpu=False) as sess:
      ...
      outputs = render_sprites.render_sprites(data=data_ph, warp=warp_ph)
  ...
  def _test_op_backward_pass(self, on_gpu, dtype, tol):
    ...
    with self.test_session(use_gpu=on_gpu, force_gpu=False):
      ...
      output_tensor = render_sprites.render_sprites(data=data_tensor, warp=warp_tensor)
      ...
      if not on_gpu:
        ...
        output_tensor_64 = render_sprites.render_sprites(data=data_tensor_64,
                                               warp=warp_tensor_64)
  ...
  def test_op_errors(self):
    ...
    with self.test_session() as sess:
      ...
      outputs = render_sprites.render_sprites(constant_op.constant(data),
